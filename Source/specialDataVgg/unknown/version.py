...
class half:
    def __init__(self,train=True):
        ...
        if train:
            ...
            self.model.load_weights('version.h5')
...
if __name__ == '__main__':
    ...
    print(np.argmax(sp_y_train,1))

    print(sp_y_train.shape)



    #retrain_model = retrain(model, sp_x_train, sp_y_train)
    #predicted_x = retrain_model.predict(sp_x_test,50)
    predicted_x = model.predict(sp_x_train,50)
    print(np.argmax(predicted_x,1))
    print(np.argmax(sp_y_train,1))

    residuals = (np.argmax(predicted_x,1)!=np.argmax(sp_y_train,1))
